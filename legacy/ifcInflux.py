...
def validate_config(config: dict):
    ...
    if missing_fields:
        ...
        raise KeyError(
            f"Could not find {missing_fields} of the influx db. Please validate the config file."
        )
            ...
            if isinstance(field, list):
                for field_idx, cur_field in enumerate(field):
                    ...
                    if sample_cnt == 1:
                        ...
                        raise ValueError(
                            "Found different number of values and timestamps!"
                        )
                        ...
                        raise ValueError(
                            "Found miss match between timestamp count the value count."
                        )
            else:
